#!/usr/bin/env python
'''
python retest_fair.py [-f fasta_file] [-m model_dir] <-T temp_dir> <-t threads>
'''
...
def normalizeKmersByMarkov(kmerHsh):
	...
	for i in kmerHsh:
		sub = i[:len(i)/2]
		kmerHsh[i] = kmerHsh[i] / subHsh[sub]
...
def predict(kmerArr, options, antibiotic, modNum = 0):
	model = None
	if os.path.isdir(options.modelDir + antibiotic):
		mFile = ""
		if os.path.exists(options.modelDir + antibiotic + '/model.0pkl'):
			mFile = options.modelDir + antibiotic + '/model.' + str(modNum) + 'pkl'
		else:
			mFile = options.modelDir + antibiotic + '/model.' + str(modNum) + '.pkl'
		...
		if os.path.exists(options.modelDir + 'all/model.0pkl'):
			mFile = options.modelDir + 'all/model.' + str(modNum) + 'pkl'
		else:
			mFile = options.modelDir + 'all/model.' + str(modNum) + '.pkl'
	...
	dm = xgb.DMatrix(fName, missing = -1, silent = True)
	# dm = xgb.DMatrix(kmerArr, missing = -1)
	# dm = xgb.DMatrix(options.tempDir + 'temp.libsvm')
...
trueLst = glob.glob(options.modelDir + 'all/model.*.true')
